...
/** Maps CMS gallery / past-event rows into the shared EventCard shape. */
export function mapPastEvent(row) {
  const description =
    (row.description && String(row.description).trim()) ||
    `Highlights from ${row.title || 'this past event'}, celebrating AMP India Foundation’s community impact.`;

  return {
    id: row.id,
    title: row.title,
    category: row.category,
    date: row.event_date || row.year,
    time: row.event_time || null,
    venue: row.venue || row.location,
    description,
    image: resolveMediaUrl(row.image_url),
    registrationLink: row.registration_link || row.details_link || null,
  };
}
